...
mesh, cell_tags, _ = gmshio.read_from_msh(
    file_name, comm, rank=0, gdim=2)  # MUST BE A .msh file for dolfinx

W = dolfinx.fem.functionspace(mesh, ("DG", 0))
k = dolfinx.fem.Function(W)
k.x.array[:] = k0
# All cells that has been tagged with physical group ID 1 has 3 * k0
...
def export_function(grid, name, show_mesh=False, tessellate=False):
    ...
    if show_mesh:
        ...
        plotter.add_mesh(grid_mesh, style="wireframe",
                         line_width=0.1, color="k")
