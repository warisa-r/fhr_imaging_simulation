...
k_0_func = dolfinx.fem.Function(W)
k_0_func.x.array[:] = k0
k_real.x.array[:] = k0
...
uinc_real.interpolate(lambda x: u_inc_green_real(x)) # approx works better for splitting!
...
g_real = (ufl.dot(ufl.grad(uinc_real), n) + k_0_func * uinc_imag)
g_imag = (ufl.dot(ufl.grad(uinc_imag), n) - k_0_func * uinc_real)
...
a = (
    (-ufl.inner(ufl.grad(u_real), ufl.grad(v_real)) * ufl.dx
     + k_0_func**2 * ufl.inner(u_real, v_real) * ufl.dx
     - k_0_func * ufl.inner(u_imag, v_real) * ufl.ds)
    
    + (-ufl.inner(ufl.grad(u_imag), ufl.grad(v_imag)) * ufl.dx
       + k_0_func**2 * ufl.inner(u_imag, v_imag) * ufl.dx
       + k_0_func * ufl.inner(u_real, v_imag) * ufl.ds)  # Absorbing Robin BC
)

# Correct source term for scattered field
L = (
    ufl.inner((k_0_func**2 - k_real**2) * uinc_real, v_real) * ufl.dx +
    ufl.inner((k_0_func**2 - k_real**2) * uinc_imag, v_imag) * ufl.dx +
    ufl.inner(g_real, v_real) * ufl.ds + ufl.inner(g_imag, v_imag) * ufl.ds
)
