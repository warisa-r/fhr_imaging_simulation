...
with XDMFFile("meshes/square_with_circle_obstacle.xdmf") as infile:
...
with XDMFFile("meshes/square_with_circle_obstacle_facets.xdmf") as infile:
...
for i, angle in enumerate(angles):
    ...
    uinc_re_neg = Expression("-AMP * cos(k_background * (cos(angle) * x[0] + sin(angle) * x[1]))",
                            AMP=AMP, k_background=k_background, angle=angle_rad,
                            degree=3)

    uinc_im_neg = Expression("-AMP * sin(k_background * (cos(angle) * x[0] + sin(angle) * x[1]))",
                            AMP=AMP, k_background=k_background, angle=angle_rad,
                            degree=3)

    uinc_re = Expression("AMP * cos(k_background * (cos(angle) * x[0] + sin(angle) * x[1]))",
                            AMP=AMP, k_background=k_background, angle=angle_rad,
                            degree=3)

    uinc_im = Expression("AMP * sin(k_background * (cos(angle) * x[0] + sin(angle) * x[1]))",
                            AMP=AMP, k_background=k_background, angle=angle_rad,
                            degree=3)
    ...
    #V_CG5 = FunctionSpace(mesh, "CG", 5)
    #u_tot_mag = project(u_tot_mag, V_CG5)
...
msh_file_path = "meshes/square_with_circle_obstacle.msh"
goal_geometry_msh_path = "meshes/square_with_kite_obstacle.msh"
